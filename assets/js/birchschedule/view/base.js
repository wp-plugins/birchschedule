(function($){
    ...
    defineFunction(ns, 'ifShowDayForDatepicker', function(date, staffId, locationId){
        ...
        var showByDaysOff = false;
        _.each(staffIds, function(staffId) {
            showByDaysOff = showByDaysOff || birchschedule.model.isDayAvaliableByDaysOff(date, staffId);
        });
        if(!showByDaysOff) {
            return [false, ""];
        }
        var showBySchedules = false;
        _.each(staffIds, function(staffId) {
            showBySchedules = showBySchedules || 
                birchschedule.model.isDayAvaliableBySchedules(date, staffId, 
                    locationId, day);
        });
        if(!showBySchedules) {
            return [false, ""];
        ...
        var datepickerOptions = $.extend(datepickerI18nOptions, {
            ...
            beforeShowDay: function(date){
                if(config.ifOnlyShowAvailable()) {
                    ...
                    var result = config.ifShowDayForDatepicker(date, staffId, locationId);
                    var dateValue = $('#' + config.dateFieldId).val();

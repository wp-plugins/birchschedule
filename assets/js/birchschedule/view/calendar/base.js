(function($){
    ...
    defineFunction(ns, 'activeViewButton', function(view){
        var selector = 'input[name="birs_calendar_view_choice"][value="' + view + '"]';
        $(selector).parent().button('toggle');
    });

    defineFunction(ns, 'parseHash', function(){
        var hash = document.location.hash.substring(1);
        var params = birchpress.util.parseParams(hash);
        return params;
    });

    defineFunction(ns, 'makeHash', function(){
    ...
    defineFunction(ns, 'onHashChange', function(){
    ...
    defineFunction(ns, 'changeHash', function(){
        ...
        var fcOptions = $.extend(fcI18nOptions, {
            ...
            eventClick: function(calEvent, jsEvent, view){
                ...
                var aptEditUrl = birchschedule.model.getAdminUrl() + 'post.php?post=' 
                    + calEvent.id + '&action=edit';
                var hash = document.location.hash.substring(1);
                if(hash) {
                    aptEditUrl = aptEditUrl + '&' + hash;   
                }
                window.location = aptEditUrl;                
            },
    ...
    defineFunction(ns, 'renderCalendar', function(params){
        ...
        $('#birs_calendar').fullCalendar('gotoDate', currentDate);

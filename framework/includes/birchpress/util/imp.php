...
class Birchpress_Util_Imp {

    private static $date_time_format_php_pattern;

    private function __construct() {
    }

    static function init_vars() {
        self::$date_time_format_php_pattern = array(
            //day of month
    ...
    static function date_time_format_php_to_jquery($date_time_format) {
        $pattern = self::$date_time_format_php_pattern;
        $replace = array(
            'dd','d',
            'DD','D',
            'MM','M','m','mm',
    ...
    static function date_time_format_php_to_fullcalendar($date_time_format) {
        $pattern = self::$date_time_format_php_pattern;
        $replace = array(
            'dd','d',
            'dddd','ddd',
    ...
    static function wp_format_time($datetime) {
        global $birchbase;
        return $birchbase->util->wp_format_time($datetime);
    }

    static function wp_format_date($datetime) {
        global $birchbase;
        return $birchbase->util->wp_format_date($datetime);
    }

    static function convert_to_datetime($timestamp) {
        global $birchbase;
        return $birchbase->util->convert_to_datetime($timestamp);
    }

    static function get_datetime_separator() {
        global $birchbase;
        return $birchbase->util->get_datetime_separator();
    }

    static function get_wp_timezone() {
        global $birchbase;
        return $birchbase->util->get_wp_timezone();
    }

    static function date_i18n($dateformatstring, $unixtimestamp) {
        global $birchbase;
        return $birchbase->util->date_i18n($dateformatstring, $unixtimestamp);
    }

    static function get_day_minutes($datetime) {
        $time = $datetime->format('H') * 60 + $datetime->format('i');
        return $time;
    }

    static function has_shortcode($shortcode = NULL) {
    ...
    static function to_mysql_date($arg) {
        global $birchbase;
        return $birchbase->util->to_mysql_date($arg);
    }

    static function get_wp_datetime($arg) {
        global $birchbase;
        return $birchbase->util->get_wp_datetime($arg);
    }
    
    static function get_weekdays_short() {
        return array(
    ...
    static function get_calendar_views() {
        return array(
    ...
    static function get_states() {
        ...
        array(
            'US' => array(
                ...
                'CT' => __('Conneticut (CT)', 'birchschedule'),
    ...
    static function convert_mins_to_time_option($mins) {
        $hour = $mins / 60;
        $min = $mins % 60;
    ...
    static function get_time_options($interval = 15) {
        $options = array();
        $value = 0;
        $format1 = '%d:%02d AM';
        $format2 = '%d:%02d PM';
    ...
    static function get_client_title_options() {
    ...
    static function get_gmt_offset() {
        return -round(self::get_wp_datetime(time())->getOffset() / 60);
    }

    static function render_html_options($options, $selection = false, $default = false) {
    ...
    static function get_first_day_of_week() {
        return get_option('start_of_week', 0);
    }
    
    static function get_fullcalendar_i18n_params() {
        global $birchpress;

        return array(
            'firstDay' => $birchpress->util->get_first_day_of_week(),
            'monthNames'=> array(
            ...
            'timeFormat' => $birchpress->util->date_time_format_php_to_fullcalendar(get_option('time_format', 'g:i a')),
            'axisFormat' => $birchpress->util->date_time_format_php_to_fullcalendar(get_option('time_format', 'g:i a'))
        );
    }
    
    static function get_datepicker_i18n_params() {
        global $birchpress;
        ...
        return array(
            ...
            'dateFormat' => $birchpress->util->date_time_format_php_to_jquery(get_option('date_format'))
        );
    }
    ...
    {
        global $birchbase;
        return $birchbase->util->starts_with($haystack, $needle);
    }
    
    static function ends_with($haystack, $needle)
    {
        global $birchbase;
        return $birchbase->util->ends_with($haystack, $needle);
    }
    ...
    static function new_errors($code = '', $message = '', $data = '') {
        return new WP_Error($code, $message, $data);
    }

    static function add_error($errors, $code, $message, $data = '') {
        $errors->add($code, $message, $data);
        return $errors;
    }

    static function get_error_codes($errors) {
        return $errors->get_error_codes();
    }

    static function get_error_code($errors) {
        return $errors->get_error_code();
    }

    static function get_error_message($errors, $code) {
        return $errors->get_error_message($code);
    }

    static function is_errors($errors) {
        return is_wp_error($errors);
    }

    static function merge_errors() {
        global $birchpress;

        $errors = $birchpress->util->new_errors();
        $args = func_get_args();
        foreach($args as $arg) {
            if($birchpress->util->is_errors($arg)) {
                $codes = $birchpress->util->get_error_codes();
                foreach($codes as $code) {
                    $message = $birchpress->util->get_error_message($code);
                    $birchpress->util->add_error($errors, $code, $message);
                }
            } 

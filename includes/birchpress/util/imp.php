...
class Birchpress_Util_Imp {
    ...
    static function wp_format_date($datetime) {
        ...
        return self::date_i18n($date_format, $timestamp);
    }
    ...
    static function convert_to_datetime($timestamp) {
        ...
        $date_format = get_option( 'date_format' );
        $time_format = get_option( 'time_format' );
        $datetime = self::get_wp_datetime($timestamp);
        $datetime_separator = $birchpress->util->get_datetime_separator();
        return self::date_i18n($date_format, $timestamp) . $datetime_separator
    ...
    static function date_i18n($dateformatstring, $unixtimestamp) {
        global $wp_locale;
        $datetime = self::get_wp_datetime($unixtimestamp);
        ...
        if ( preg_match( "/$timezone_formats_re/", $dateformatstring ) ) {
            $timezone_object = self::get_wp_timezone();
    ...
    static function get_wp_datetime($arg) {
        ...
        if (is_array($arg)) {
            $datetime = self::to_mysql_date($arg);

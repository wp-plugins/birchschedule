...
class BIRS_Calendar_View extends BIRS_Admin_View {
    ...
    function validate_datetime() {
        ...
        $staff->load();
        $available_times = $staff->get_avaliable_time($location_id, $service_id, $date);
        if(array_key_exists($time, $available_times)) {
            if($available_times[$time]['avaliable']) {
                return true;
            } else {
                return false;
            }
    ...
    function validate_appointment_info() {
        ...
        if(!isset($errors['birs_appointment_service']) && !isset($errors['birs_appointment_datetime'])) {
            if(!$this->validate_datetime()) {
                $errors['birs_appointment_datetime'] = __('Date & time is unavailable', 'birchschedule');
            }
        }
        return $errors;
    }

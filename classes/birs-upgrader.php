...
class BIRS_Upgrader {
    ...
    function upgrade_service_settings_from_1_0_to_1_1() {
        $version = $this->get_db_version_service_settings();
        if($version != '1.0') {
            return;
        }
        $query = new BIRS_Model_Query(
        ...
        foreach ($services as $service_id => $service) {
            $old_service_title = 
                $birchschedule->shortcode->get_service_option_text_1_0($service);
            $service->post_title = $old_service_title;
            $service->save();
        }
        update_option('birs_db_version_service_settings', '1.1');
    }

    function get_db_version_service_settings() {
        return get_option('birs_db_version_service_settings', '1.0');
    }
    
    function upgrade_core() {
        $this->upgrade_staff_schedule_from_1_0_to_1_1();
        $this->upgrade_service_settings_from_1_0_to_1_1();
    }
